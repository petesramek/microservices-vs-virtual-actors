...
Group contracts by business capability, then separate requests and responses when a capability contains several types.

```text
Workbench.Contracts/
├── Workbench.Contracts.csproj
├── README.md
├── Inventory/
│   ├── Requests/
│   │   ├── ReleaseInventoryRequest.cs
│   │   ├── ReserveInventoryRequest.cs
│   │   └── ResetInventoryRequest.cs
│   └── Responses/
│       ├── InventoryResponse.cs
│       └── ReserveInventoryResponse.cs
├── Orders/
│   ├── OrderResponse.cs
│   └── OrderStatus.cs
└── Payments/
    ├── AuthorizePaymentRequest.cs
    └── AuthorizePaymentResponse.cs
```
