...
Generated `bin` and `obj` directories, the user-specific `.csproj.user` file, compiled outputs, and generated static web assets are intentionally omitted.

The structure below reflects the current source tree together with the component renames completed during the UI cleanup.
...
├── Internal/
│   ├── Clients/
│   │   └── ScenarioRunnerClient.cs
│   ├── Extensions/
│   │   └── SystemHealthServiceCollectionExtensions.cs
│   └── Observability/
│       ├── Health/
│       │   ├── HealthEndpointOptions.cs
│       │   └── SystemHealthService.cs
│       └── Topology/
│           ├── TopologyDefinitionProvider.cs
│           └── TopologyOptions.cs
├── Models/
│   └── ScenarioFormModel.cs
├── Properties/
│   └── launchSettings.json
├── wwwroot/
│   ├── app.css
...
Keep this tree synchronized with subsequent feature-folder consolidation. In particular, `ResultCard`, `ScenarioFormModel`, and `ScenarioRunnerClient` are scenario-specific types and may move together when the scenario feature is reorganized.
