...
using Orders.Api.Observability.Health;
...
builder.Services.AddHttpClient<IInventoryClient, HttpInventoryClient>(client => {
    var baseUrl = builder.Configuration["Services:InventoryBaseUrl"]
        ?? "http://localhost:5201";
...
builder.Services.AddHttpClient<IPaymentsClient, HttpPaymentsClient>(client => {
    var baseUrl = builder.Configuration["Services:PaymentsBaseUrl"]
        ?? "http://localhost:5202";
