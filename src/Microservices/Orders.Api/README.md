...
Generated `bin` and `obj` directories, the user-specific `.csproj.user` file, and the local `orders.db`, `orders.db-shm`, and `orders.db-wal` artifacts are intentionally omitted.
...
│   │   │   ├── IInventoryClient.cs
│   │   │   └── IPaymentsClient.cs
│   │   ├── HttpInventoryClient.cs
│   │   └── HttpPaymentsClient.cs
│   ├── Infrastructure/
│   │   └── OrdersDbContext.cs
│   └── Observability/
│       ├── Health/
│       │   ├── InventoryApiHealthCheck.cs
│       │   ├── OrdersDatabaseHealthCheck.cs
│       │   └── PaymentsApiHealthCheck.cs
│       └── Logging/
│           ├── LogError.cs
│           └── LogInformation.cs
├── Models/
